use nanobot::ldtab::{get_label_map, get_prefix_map, get_property_map, get_predicate_map_hiccup};
...
async fn test_get_predicate_map_hiccup() {
    ...
    let hiccup = get_predicate_map_hiccup(&subject, &table, &pool).await.unwrap();
